...
class TranslationService:
    ...
    def __init__(self, cache_enabled: bool = True):
        ...
        logger.info(f"Translation service initialized with {len(self._get_available_backends())} backends")
    ...
    def translate_text(self, text: str, target_lang: str = 'en', source_lang: str = 'auto') -> Dict:
        """
        ...
        for backend_name, translate_func in backends_to_try:
            try:
                ...
                if result and result.get('translation'):
                    ...
                    if self.cache:
                        self.cache.set(text, source_lang, target_lang, result['translation'])
    ...
    def _translate_googletrans(self, text: str, source_lang: str, target_lang: str) -> Dict:
        ...
        try:
            ...
            if hasattr(result, '__await__'):
                ...
                raise Exception("googletrans 4.0+ requires async handling - using fallback")
    ...
    def _translate_deep_google(self, text: str, source_lang: str, target_lang: str) -> Dict:
        ...
        translator = self.deep_translators['google'](source=source_lang, target=target_lang)
    ...
    def _translate_mymemory(self, text: str, source_lang: str, target_lang: str) -> Dict:
        ...
        translator = self.deep_translators['mymemory'](source=source_lang, target=target_lang)
        translation = translator.translate(text)
    ...
    def translate_health_keywords(self) -> Dict[str, Dict[str, str]]:
        ...
        for lang_code, lang_name in self.TARGET_LANGUAGES.items():
            ...
            for keyword in keywords:
                try:
                    result = self.translate_text(keyword, target_lang=lang_code, source_lang='en')
                    if result.get('translation') and not result.get('error'):
                        ...
                        logger.warning(f"  Failed to translate '{keyword}' to {lang_name}")
                        translations[lang_code][keyword] = keyword  # Fallback to English
